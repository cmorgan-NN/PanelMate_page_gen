...
class Static_Graphic_Data_Rectangle:
    ...
    def render_visual_order(visual_order):
        ...
        return_list.extend(['rectangle_element_' + vo + ' = Rectangle(screen, ' + x_origin +
                            ', ' + y_origin +
                            ', ' + size_x + ' - 1 ' +
                            ', ' + size_y + ' - 1 ' + ')'])
        return_list.extend(['rectangle_element_' + vo + '.color(Color(panelMateColorTo24Bit(' + 
                            fill_color + ')))']) # fill_color
        return_list.extend(['rectangle_element_' + vo + '.border(Color(panelMateColorTo24Bit(' +
                            pen_color + ')))']) # pen_color
        return_list.extend(['rectangle_element_' + vo + '.border_width(' + pen_width + ')']) # pen_width
        return_list.extend([''])
