...
class Variable_Sized_Indicator:
    ...
    def parse_visual_orders(visual_order_elements):
        ...
        for visual_order_element in visual_order_elements:
            ...
            for line in visual_order_element:  # Parse each Visual Order values
                ...
                if not elements_working:
                    ...
                    if "pos_conditional_expression" in locals():
                        val_conditional_expression = line[
                            pos_conditional_expression:
                        ].strip("\n")
                        line = line[
                            :pos_alarm
                        ]  # remove extracted values from current line
                    ...
                    if (
                        "val_alarm_message" in locals()
                        and "val_pen_color" in locals()
                        and "val_fill_color" in locals()
                        and "val_alarm" in locals()
                        and "val_alarmAck" in locals()
                        and "val_conditional_expression" in locals()
                    ):
                        visual_order.update(
                            {
                                "Indicator States": {
                ...
                if elements_working:
                    for member in enumerate(
                        elements_working[:]
                    ):  # enumerate copied ([:]) elements to loop through them in order they're stored
                        if member[1] in line:
                            if len(elements_working) > 1:
                                parsed_element = Sub_Parsers.element_parser(
                                    member[1],
                                    elements_working[
                                        elements_working.index(member[1]) + 1
                                    ],
                                    line,
                                )
                            else:
            ...
            visual_orders.update(
                {visual_order["Visual Order"]: visual_order}
